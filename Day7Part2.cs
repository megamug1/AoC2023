...
public class Day7Part2
{
    private class Hand : IComparable
    {
        ...
        public void Init()
        {
            ...
            if(jCount == 5) handType = HandType.FIVE_OF_A_KIND;
            else if(jCount == 4) handType = HandType.FIVE_OF_A_KIND;
            else if(jCount == 3)
            {
                if(counts.Contains(2)) handType = HandType.FIVE_OF_A_KIND;
                else handType = HandType.FOUR_OF_A_KIND;
            }
            else if(jCount == 2)
            {
                if(counts.Contains(3)) handType = HandType.FIVE_OF_A_KIND;
                else if(counts.Contains(2)) handType = HandType.FOUR_OF_A_KIND;
                else handType = HandType.THREE_OF_A_KIND;
            }
            else if(jCount == 1)
            {
                if(counts.Contains(4)) handType = HandType.FIVE_OF_A_KIND;
                else if(counts.Contains(3)) handType = HandType.FOUR_OF_A_KIND;
                else if(Array.FindAll(counts, count => count == 2).Length == 2) handType = HandType.FULL_HOUSE;
                else if(counts.Contains(2)) handType = HandType.THREE_OF_A_KIND;
